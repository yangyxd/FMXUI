...
type
  ...
  private
    ...
    procedure ContextBeforeLosingHandler(const Sender: TObject;
      const Msg: TMessage);
  ...
  private
    FContext: TContext3D;
  public
    procedure DrawTexture(const ARect, ATexRect: TRectF;
      const AColor: TAlphaColor; const ATexture: TTexture); virtual; abstract;
    class property ModulateColor: TAlphaColor read FModulateColor
      write FModulateColor;
    class property AlignToPixels: Boolean read FAlignToPixels
      write FAlignToPixels;
...
{$IFDEF MSWINDOWS}
  FMX.Platform.Win,
{$ENDIF}
  System.Math, FMX.Consts, FMX.Platform, FMX.Forms, FMX.TextLayout,
...
type
  ...
  private
    ...
    procedure InternalFillPolygon(const Points: TPolygon;
      const AOpacity: Single; const ABrush: TBrush; const PolyBounds: TRectF);
  ...
  protected
    ...
    procedure DoFillEllipse(const ARect: TRectF; const AOpacity: Single;
      const ABrush: TBrush); override;
    function DoFillPolygon(const Points: TPolygon; const AOpacity: Single;
      const ABrush: TBrush): Boolean; override;
    ...
    procedure DoDrawRect(const ARect: TRectF; const AOpacity: Single;
      const ABrush: TStrokeBrush); override;
    function DoDrawPolygon(const Points: TPolygon; const AOpacity: Single;
      const ABrush: TStrokeBrush): Boolean; override;
  ...
  public
    ...
    function PtInPath(const APoint: TPointF; const APath: TPathData)
      : Boolean; override;
    { external }
    procedure DrawTexture(const DstRect, SrcRect: TRectF;
      const AColor: TAlphaColor; const ATexture: TTexture); override;
  end;
...
constructor TBitmapCtx.Create(const AWidth, AHeight: Integer;
  const AScale: Single);
begin
  inherited Create;
  FHeight := AHeight;
  ...
  if FPixelFormat = TPixelFormat.None then
    raise ECannotFindSuitablePixelFormat.CreateResFmt
      (@SCannotFindSuitablePixelFormat, [ClassName]);
  FBytesPerLine := FWidth * PixelFormatBytes[FPixelFormat];
  FAccess := TMapAccess.Read;
  FContextBeforeLosingId := TMessageManager.DefaultManager.SubscribeToMessage
    (TContextBeforeLosingMessage, ContextBeforeLosingHandler);
...
begin
  TMessageManager.DefaultManager.Unsubscribe(TContextLostMessage,
    FContextLostId);
  TMessageManager.DefaultManager.Unsubscribe(TContextBeforeLosingMessage,
    FContextBeforeLosingId);
  if (TCanvasGpu.FCanvasHelper <> nil) and (TCanvasGpu.FGlobalBeginScene > 0)
  then
...
procedure TBitmapCtx.ContextBeforeLosingHandler(const Sender: TObject;
  const Msg: TMessage);
begin
{$IFNDEF ANDROID}
  if (FTexture = nil) and (FRenderTarget <> nil) and (FBytesPerLine > 0) and
    (FCanvas <> nil) and (FCanvas.Context <> nil) then
  begin
    CreateBuffer;
    FCanvas.Context.CopyToBits(FData, FBytesPerLine, TRect.Create(0, 0, FWidth,
      FHeight));
  end;
{$ENDIF}
end;

procedure TBitmapCtx.ContextLostHandler(const Sender: TObject;
  const Msg: TMessage);
begin
{$IFNDEF ANDROID}
  if (FTexture = nil) and (FRenderTarget <> nil) and (FBytesPerLine > 0) and
...
function PrepareColor(const SrcColor: TAlphaColor; const Opacity: Single)
  : TAlphaColor;
begin
  if Opacity < 1 then
  begin
...
constructor TCanvasGpu.CreateFromWindow(const AParent: TWindowHandle;
  const AWidth, AHeight: Integer;
...
begin
  ...
  case Quality of
    ...
    TCanvasQuality.HighQuality:
      Multisample := TMultisample.FourSamples;
  else
    Multisample := TMultisample.None;
  end;
  FContext := TContextManager.CreateFromWindow(AParent, AWidth, AHeight,
    Multisample, True);
{$IFDEF MSWINDOWS}
...
begin
  ...
  FContext := TContextManager.CreateFromTexture(H.RenderTarget,
    Multisample, True);
  H.Canvas := Self;
end;
...
begin
  ...
  begin
    FCanvasHelper.Flush;
    FContext.Clear([TClearTarget.Color, TClearTarget.Depth,
      TClearTarget.Stencil], Color, 1, 0);
  end;
end;
...
begin
  ...
  Result := inherited DoBeginScene(AClipRects) and (FContext <> nil) and
    FContext.BeginScene;
  if Result then
  begin
...
{$IFDEF MSWINDOWS}
    if (Parent <> nil) and WindowHandleToPlatform(Parent).Form.Transparency then
      Context.CopyToBits(WindowHandleToPlatform(Parent).BufferBits, Width * 4,
...
begin
  ...
  Intersection := TRect.Intersect(FCurrentClipRect,
    Rect(Trunc(TopLeft.X), Trunc(TopLeft.Y), Round(BottomRight.X),
    Round(BottomRight.Y)));
...
function GetStrokeOpacity(const AOpacity: Single;
  const ABrush: TStrokeBrush): Single;
const
...
begin
  if ABrush.Thickness < 1 then
    if ABrush.Thickness < MinValidStrokeThickness then
      ...
      Result := AOpacity * (ABrush.Thickness - MinValidStrokeThickness) /
        (1 - MinValidStrokeThickness)
  else
    Result := AOpacity;
end;

procedure TCanvasGpu.DoDrawLine(const APt1, APt2: TPointF;
...
begin
  ...
  begin
    ...
    begin
      FCanvasHelper.FillTriangles(FStrokeBuilder.Vertices,
        FStrokeBuilder.Colors, FStrokeBuilder.Indices,
...
procedure TCanvasGpu.DoDrawRect(const ARect: TRectF; const AOpacity: Single;
...
begin
  ...
  begin
    ...
    FStrokeBuilder.BuildLine(ARect.TopLeft, ARect.TopLeft + PointF(ARect.Width,
      0.0), LOpacity);
    ...
    FStrokeBuilder.BuildLine(ARect.TopLeft + PointF(ARect.Width, 0.0),
      ARect.BottomRight, LOpacity);
    ...
    FStrokeBuilder.BuildLine(ARect.BottomRight, ARect.TopLeft + PointF(0.0,
      ARect.Height), LOpacity);
    ...
    FStrokeBuilder.BuildLine(ARect.TopLeft + PointF(0.0, ARect.Height),
      ARect.TopLeft, LOpacity);
...
procedure TCanvasGpu.DoDrawEllipse(const ARect: TRectF; const AOpacity: Single;
  const ABrush: TStrokeBrush);
var
  LOpacity: Single;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ABrush.Dash <> TStrokeDash.Solid) and
        (ABrush.Dash <> TStrokeDash.Custom) then
      ...
      begin
        FCanvasHelper.FillTriangles(FStrokeBuilder.Vertices,
          FStrokeBuilder.Colors, FStrokeBuilder.Indices,
          Length(FStrokeBuilder.Vertices),
...
function TCanvasGpu.DoDrawPolygon(const Points: TPolygon;
  const AOpacity: Single; const ABrush: TStrokeBrush): Boolean;
var
  LOpacity: Single;
begin
  LOpacity := GetStrokeOpacity(AOpacity, ABrush);
  ...
  begin
    ...
    if (ABrush.Dash <> TStrokeDash.Solid) and (ABrush.Dash <> TStrokeDash.Custom)
    then
...
procedure TCanvasGpu.DoDrawPath(const APath: TPathData; const AOpacity: Single;
  const ABrush: TStrokeBrush);
var
  LOpacity: Single;
begin
...
procedure TCanvasGpu.DoFillRect(const ARect: TRectF; const AOpacity: Single;
  const ABrush: TBrush);
var
  B: TBitmapCtx;
  Bmp: TBitmap;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FCanvasHelper.GradientRect(TransformBounds(CornersF(ARect)),
          ABrush.Gradient);
    end
    ...
    begin
      ...
      begin
        ...
        case ABrush.Bitmap.WrapMode of
          TWrapMode.Tile:
            FCanvasHelper.TexQuad(TransformPoint(PointF(ARect.Left, ARect.Top)),
              TransformPoint(PointF(ARect.Right, ARect.Top)),
              TransformPoint(PointF(ARect.Right, ARect.Bottom)),
              TransformPoint(PointF(ARect.Left, ARect.Bottom)), PointF(0, 0),
              PointF(ARect.Width / Bmp.Width, 0),
              PointF(ARect.Width / Bmp.Width, ARect.Height / Bmp.Height),
              PointF(0, ARect.Height / Bmp.Height), PrepareColor(FModulateColor,
              AOpacity), B.PaintingTexture);
          TWrapMode.TileOriginal:
            begin
              DstRect := RectF(0, 0, Bmp.Width, Bmp.Height);
              IntersectRect(DstRect, DstRect, ARect);
              FCanvasHelper.TexRect(TransformBounds(CornersF(DstRect)),
                CornersF(DstRect), B.PaintingTexture,
                PrepareColor(FModulateColor, AOpacity));
            end;
          TWrapMode.TileStretch:
            FCanvasHelper.TexRect(TransformBounds(CornersF(ARect)),
              B.PaintingTexture, PrepareColor(FModulateColor, AOpacity));
        end;
      end;
    end
    else
      FCanvasHelper.FillRect(TransformBounds(CornersF(ARect)),
        PrepareColor(ABrush.Color, AOpacity));
  end;
end;
...
procedure TCanvasGpu.DoFillEllipse(const ARect: TRectF; const AOpacity: Single;
  const ABrush: TBrush);
var
  B: TBitmapCtx;
  Bmp: TBitmap;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FCanvasHelper.GradientEllipse(ARect, ABrush.Gradient,
          TransformCallback);
    end
    else if ABrush.Kind = TBrushKind.Bitmap then
    begin
      ...
      begin
        ...
        case ABrush.Bitmap.WrapMode of
          TWrapMode.Tile:
            FCanvasHelper.TexEllipse(ARect,
              TRectF.Create(0, 0, ARect.Width / Bmp.Width,
              ARect.Height / Bmp.Height), PrepareColor(FModulateColor,
              AOpacity), B.PaintingTexture, TransformCallback);
          TWrapMode.TileOriginal:
            begin
              DstRect := TRectF.Create(0, 0, Bmp.Width, Bmp.Height);
              IntersectRect(DstRect, DstRect, ARect);
              FCanvasHelper.TexEllipse(ARect,
                RectF(0, 0, ARect.Width / DstRect.Width,
                ARect.Height / DstRect.Height), PrepareColor(FModulateColor,
                AOpacity), B.PaintingTexture, TransformCallback);
            end;
          TWrapMode.TileStretch:
            FCanvasHelper.TexEllipse(ARect, PrepareColor(FModulateColor,
              AOpacity), B.PaintingTexture, TransformCallback);
        end;
      end;
    end
    else
      FCanvasHelper.FillEllipse(ARect, PrepareColor(ABrush.Color, AOpacity),
        TransformCallback);
  end;
end;

procedure TCanvasGpu.MeasureLines(const ALines: TLineMetricInfo;
...
class function TCanvasGpu.DoInitializeBitmap(const Width, Height: Integer;
...
class function TCanvasGpu.DoMapBitmap(const Bitmap: THandle;
...
begin
  ...
  begin
    if Handle.Canvas <> nil then
      TCanvasGpu(Handle.Canvas).Context.CopyToBits(Data.Data, Data.Pitch,
        TRect.Create(0, 0, Handle.Width, Handle.Height))
    else if Handle.FRenderTarget <> nil then
    begin
      Context := TContextManager.CreateFromTexture(Handle.FRenderTarget,
        TMultisample.None, False);
      try
        Context.CopyToBits(Data.Data, Data.Pitch,
          Rect(0, 0, Handle.Width, Handle.Height))
      finally
        Context.DisposeOf;
      end;
    end;
...
class procedure TCanvasGpu.DoUnmapBitmap(const Bitmap: THandle;
...
procedure TCanvasGpu.DrawTexture(const DstRect, SrcRect: TRectF;
  const AColor: TAlphaColor; const ATexture: TTexture);
begin
  FCanvasHelper.TexRect(TransformBounds(CornersF(DstRect)),
    CornersF(SrcRect.Left, SrcRect.Top, SrcRect.Width, SrcRect.Height),
    ATexture, $FFFFFFFF);
end;

procedure TCanvasGpu.DoDrawBitmap(const ABitmap: TBitmap;
  const SrcRect, DstRect: TRectF; const AOpacity: Single;
...
function TCanvasGpu.PtInPath(const APoint: TPointF;
    ...
    if APoint.Y > Min(Temp1.Y, Temp2.Y) then
      if APoint.Y <= Max(Temp1.Y, Temp2.Y) then
        if APoint.X <= Max(Temp1.X, Temp2.X) then
          ...
          begin
            if SameValue(Temp1.X, Temp2.X, TEpsilon.Vector) or
              (APoint.X <= (APoint.Y - Temp1.Y) * (Temp2.X - Temp1.X) /
              (Temp2.Y - Temp1.Y) + Temp1.X) then
              Result := not Result;
          end;
...
procedure TCanvasGpu.InternalFillPolygon(const Points: TPolygon;
  const AOpacity: Single; const ABrush: TBrush; const PolyBounds: TRectF);
var
  DstRect: TRectF;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ABrush.Kind = TBrushKind.Bitmap) and
        (ABrush.Bitmap.WrapMode = TWrapMode.Tile) and (Bmp <> nil) and not Bmp.IsEmpty
      then
        TexCoords[PlaceAt] := TPointF.Create((Point.X - PolyBounds.Left) /
          Bmp.Width, (Point.Y - PolyBounds.Top) / Bmp.Height)
      else
        TexCoords[PlaceAt] := TPointF.Create((Point.X - PolyBounds.Left) /
          PolyBounds.Width, (Point.Y - PolyBounds.Top) / PolyBounds.Height);

      if InitIndex = -1 then
        InitIndex := CurIndex;

      if (InitIndex <> -1) and (PrevIndex <> -1) and (InitIndex <> PrevIndex)
      then
      begin
  ...
  begin
    ...
    FCanvasHelper.FillTriangles(Vertices, Colors, Indices, Length(Vertices),
  ...
  begin
    ...
    begin
      ...
      Gradient.Assign(ABrush.Gradient);
      Gradient.ApplyOpacity(AOpacity);
      FCanvasHelper.GradientTriangles(Gradient, Vertices, TexCoords, Indices,
        Length(Vertices), Length(Indices) div 3);
      Gradient.Free;
    end
    else
      FCanvasHelper.GradientTriangles(ABrush.Gradient, Vertices, TexCoords,
        Indices, Length(Vertices), Length(Indices) div 3)
  end
  else if (ABrush.Kind = TBrushKind.Bitmap) and (Bmp <> nil) and Bmp.HandleAllocated
  then
  begin
    B := TBitmapCtx(Bmp.Handle);
    case ABrush.Bitmap.WrapMode of
      TWrapMode.Tile:
        FCanvasHelper.TexTriangles(B.PaintingTexture, Vertices, TexCoords,
          Colors, Indices, Length(Vertices), Length(Indices) div 3);
      TWrapMode.TileOriginal:
        begin
          DstRect := TRectF.Create(0, 0, Bmp.Width, Bmp.Height);
          IntersectRect(DstRect, DstRect, PolyBounds);
          FCanvasHelper.TexRect(TransformBounds(CornersF(DstRect)),
            CornersF(DstRect), B.PaintingTexture, PrepareColor(FModulateColor,
            AOpacity));
        end;
      TWrapMode.TileStretch:
        FCanvasHelper.TexTriangles(B.PaintingTexture, Vertices, TexCoords,
    ...
    FCanvasHelper.FillTriangles(Vertices, FillColors, Indices, Length(Vertices),
      Length(Indices) div 3);
...
function TCanvasGpu.DoFillPolygon(const Points: TPolygon;
...
procedure TCanvasGpu.DoFillPath(const APath: TPathData; const AOpacity: Single;
  const ABrush: TBrush);
var
  Points: TPolygon;
begin
