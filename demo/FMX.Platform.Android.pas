...
uses
  ...
  FMX.Text, FMX.Graphics, FMX.TextLayout, FMX.Maps, System.Devices, FMX.Presentation.Style;
...
begin
  case ACmd of
    ...
    APP_CMD_LOST_FOCUS:
      begin
        HandleApplicationEvent(TApplicationEvent.WillBecomeInactive, nil);
...
begin
  ...
  else if (Abs(FLastOrientationTime - GetTick) * 1000 > PollOrientationChange) and not TWindowManager.Current.Pause then
    CheckOrientationChange;
...
begin
  ...
  // On the Android Zero index 駉rresponds Sunday, so we need to add offset. Because in RTL DayMonday = 1
      ...
      PlatformAndroid.SynchronizeOnUIThread(
        ...
        begin
          ...
          begin
            ...
            if (SelEnd - SelStart) > 0 then  // by YangYxd 改变执行顺序。不然在 D10.1 Update2 中，特定情况下会闪退
